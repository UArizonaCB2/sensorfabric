""""
    ...
    This file contains methods that are used to parse raw sensor files returned by various platforms.
"""
...
"""
    @arg - filename - Full path of the file.
...
if __name__ == '__main__':
    (ret, json_buffer) = scanJsonFile(sys.argv[1])

    # Pretty print the schema for each json.
    if ret:
        for json_data in json_buffer:
            prettyPrintSchema(json_data, showType=True)

    # Test out the query functionality.
    """
    (ret, node) = execQuery(json_buffer[0], "SELECT value FROM activities-heart-intraday.dataset")
    if ret:
        for n in node:
            print(n)
     """
